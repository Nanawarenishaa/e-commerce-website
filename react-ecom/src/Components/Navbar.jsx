...
const Navbar = () => {
  ...
  return (
    <div className="w-full lg:w-[80%] ml-4 lg:ml-36 flex gap-4 lg:gap-8 bg-transparent z-20 absolute mt-8 lg:mt-44 items-center px-4">
      <img src={toggleIcon} alt="Toggle menu" className="cursor-pointer lg:hidden" onClick={toggleMenu} />
      {isMenuOpen && (
        <div className="fixed left-0 top-0 flex flex-col justify-left pl-10 gap-6 text-lg lg:text-2xl w-48 lg:w-60 bg-black text-white h-screen pt-16 transition-all duration-300 ease-in-out">
          <RxCross2 className="absolute right-6 top-6 cursor-pointer duration-300 hover:text-red-500" onClick={toggleMenu} />
          <a href="/" className="duration-300 hover:text-red-500">Home</a>
          <a href="/" className="duration-300 hover:text-red-500">Fashion</a>
          <a href="/" className="duration-300 hover:text-red-500">Electronics</a>
          <a href="/" className="duration-300 hover:text-red-500">Jewelry</a>
        </div>
      )}
